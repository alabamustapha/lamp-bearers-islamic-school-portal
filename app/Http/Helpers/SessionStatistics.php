...
function term_percentage_done($session_id, $term){
    ...
    foreach($classrooms as $classroom){
            ...
            $total_students_scores_that_should_be_recorded += $classroom->students()->whereIn('status', ['promoting', 'repeating', 'graduating'])->where('previous_classroom_id', '=', $classroom->id)->count() * $classroom->subjects()->count();
  ...
  //  var_dump($total_students_scores_that_should_be_recorded);
    ...
    foreach($classrooms_with_results as $classroom){
        ...
        $records_count = \App\Result::where('classroom_id', '=', $classroom->id)->where('session_id', '=', $session_id)->where('term', '=', $term)->get()->count();
